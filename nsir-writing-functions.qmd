...
Write a function to calculate a person's present age given their birth year. Add the current year as a default value for the calculation.
...
```{r}
# return as a named list (or named vector)
average_and_sd_calculator <- function(x) {
  mean_x = mean(x)
  sd_x = sd(x)
  
  return(list("mean_grade" = mean_x, "sd_grade" = sd_x)) # also works!
  # Alternatively, return a named vector
  # return(c("mean_grade" = mean_x, "sd_grade" = sd_x))
}
...
Named vectors or named lists allow the user to specify a name for each element. Thus, naming takes out any ambiguity about which elements represent which values.
...
First, let's see the tidyverse way of performing this task. Suppose I want to calculate the mean of a column from a dataframe, in this case the mean of column `body_mass_g` from `penguins` dataframe.
...
Now let's try to convert the code above into a function
...
Now let's call the same expression for mean of `penguins` column `body_mass_g` using the `simple_function` we created above:
